...
from utils.youtube_utils import (
    get_uploads_playlist_id, get_recent_video_ids,
    fetch_all_comments_for_video, fetch_channel_details
)
from utils.sentiment_analysis import analyze_comments_batch
import time
...
def analyze_brand_image_compatibility(
    ...
    TODO: 실제 구현 시 CLIP 모델 또는 Vision API 사용
    현재는 더미 데이터 반환
    """
    ...
    # 임시: 카테고리 기반 적합도 계산
    # 실제로는 썸네일 이미지와 브랜드 이미지를 AI로 비교
    base_score = 75.0  # 기본 점수
    
    if influencer and influencer.category:
        # 카테고리가 일치하면 가산점
        if brand_name in influencer.title or brand_name in (influencer.description or ""):
            base_score += 15.0
        
        # 카테고리별 조정
        category_lower = influencer.category.lower()
        if "건강" in category_lower or "뷰티" in category_lower:
            base_score += 5.0
    
    return BrandImageScore(
        channel_id=channel_id,
        brand_name=brand_name,
        similarity_score=min(100.0, base_score),
        analysis_method="category_based_temp",
        thumbnail_url=influencer.thumbnail_url if influencer else None,
...
def perform_sentiment_analysis(
    ...
    # 4. 감성분석 실행
    result = analyze_comments_batch(all_comments)
...
def run_simulator(
    ...
    total_score_value = calculate_total_score(
        brand_score=brand_image.similarity_score,
    ...
    recommendation = generate_recommendation(
        total_score_value,
        {
            "brand_image": brand_image.similarity_score,
    ...
    total_score = TotalScore(
        ...
        brand_image_score=brand_image.similarity_score,
        sentiment_score=sentiment.sentiment_score,
        roi_score=roi_estimate.roi_score,
        weights=req.weights,
...
def compare_weights(
    ...
    base_req = SimulatorRequest(
        channel_id=channel_id,
        brand_name=brand_name,
        weights=weight_configs[0]  # 첫 번째 설정으로 분석
    )
    ...
    for weights in weight_configs:
        total = calculate_total_score(
            brand_score=base_result.brand_image.similarity_score,
        ...
        comparisons.append({
            ...
            "recommendation": generate_recommendation(total, {
                "brand_image": base_result.brand_image.similarity_score,
